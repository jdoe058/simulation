...
Приведение потока к списку (stream().toList()) убирает исключение ConcurrentModificationException, но появляется 
другая проблема, почему-то уменьшается количество хищников, на которых пока никто не охотится, что еще хуже.
